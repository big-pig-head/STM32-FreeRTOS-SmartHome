...
void OLED_W_SDA(uint8_t BitValue)                                      
{
	...
	GPIO_WriteBit(GPIOB, GPIO_Pin_9, (BitAction)BitValue);            //OLED_W_SDA（1），SDA置为高电平。OLED_W_SDA（0），SDA置为低电平。
...
void OLED_GPIO_Init(void)
{
	...
	/*释放SCL和SDA*/
	OLED_W_SCL(1);                            //不闭合mos管，即释放scl，sda两条总线，因为有外接上拉电阻，scl，sda就直接恢复高电平的了
	OLED_W_SDA(1);
}
...
void OLED_I2C_Start(void)
{
	...
	OLED_W_SCL(0);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接，SCL为0时，SDA可以放数据，SCL为1时。SDA读数据
}
...
void OLED_I2C_SendByte(uint8_t Byte)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		OLED_W_SCL(0);	//拉低SCL，主机开始发送下一位数据，这时候如果从机收到数据，一般会发ACK，在SCL为0是改变sda的电平，在SCL为1时主机读SDA的电平，如果为1则收到从机的ACK
